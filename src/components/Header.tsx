      ...
      <div className="container-wide flex items-center justify-between h-16 md:h-20">
        <Link
          ...
          className={`flex items-center pr-2 ${
            isGreekPage ? "gap-2 md:gap-3 lg:gap-2.5" : "gap-2.5 md:gap-3.5"
          }`}
        >
          <img
            src={logoSymbol}
        ...
        <nav className={`hidden lg:flex items-center ${isGreekPage ? "gap-6 xl:gap-7" : "gap-8"}`}>
          {navItems.map((item) => (
            <Link
              key={item.path}
              to={item.path}
          ...
          <div className={`flex items-center gap-2 transition-colors duration-500 ${
            isGreekPage ? "ml-3 pl-3 xl:ml-4 xl:pl-4" : "ml-4 pl-4"
